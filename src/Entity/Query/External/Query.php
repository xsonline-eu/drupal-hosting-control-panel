...
/**
 * @file
 * Contains \Drupal\hosting_control_panel\Entity\Query\External\Query.
 */
...
class Query extends QueryBase implements QueryInterface {
  ...
  /**
   * The parameters to send to the Hosting control panel entity storage client.
   *
   * @var array
   */
  ...
  protected function prepare() {
    if ($this->amountOfBundleConditions() !== 1) {
      throw new QueryException("You must specify a single bundle for Hosting control panel entity queries.");
    }
  ...
  protected function checkConditions(\Drupal\Core\Entity\Query\ConditionInterface $condition = NULL) {
    ...
    foreach ($condition->conditions() as $c) {
      ...
      elseif ($c['operator'] && !in_array($c['operator'], $this->supportedOperators())) {
        throw new QueryException("Operator {$c['operator']} is not supported by Hosting control panel entity queries.");
      }
    }
  }
