...
/**
 * @file
 * Contains \Drupal\hosting_control_panel\HostingControlPanelEntityTypeForm.
 */
 ...
 */
class HostingControlPanelEntityTypeForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    if ($this->operation == 'add') {
      $form['#title'] = SafeMarkup::checkPlain($this->t('Add Hosting Control Panel Entity type'));
      $base_fields = $this->entityManager->getBaseFieldDefinitions('hosting_control_panel_entity');
      $fields = $this->entityManager->getFieldDefinitions('hosting_control_panel_entity', $type->id());
      // Create an Hosting control panel entity with a fake bundle using the type's UUID so
      // that we can get the default values for workflow settings.
      // @todo Make it possible to get default values without an entity.
      //   https://www.drupal.org/node/2318187
      $node = $this->entityManager->getStorage('hosting_control_panel_entity')->create(array('type' => $type->uuid()));
    }
    else {
      $form['#title'] = $this->t('Edit %label Hosting Control Panel Entity type', array('%label' => $type->label()));
      $base_fields = $this->entityManager->getFieldDefinitions('hosting_control_panel_entity', $type->id());
      $fields = $this->entityManager->getFieldDefinitions('hosting_control_panel_entity', $type->id());
      // Create a node to get the current values for workflow settings fields.
      $node = $this->entityManager->getStorage('hosting_control_panel_entity')->create(array('type' => $type->id()));
    };
    unset($fields[$this->entityManager->getDefinition('hosting_control_panel_entity')->getKey('uuid')]);
    unset($fields[$this->entityManager->getDefinition('hosting_control_panel_entity')->getKey('bundle')]);

    $form['label'] = array(
      '#title' => $this->t('Name'),
      '#type' => 'textfield',
      '#default_value' => $type->label(),
      '#description' => $this->t('The human-readable name of this Hosting Control Panel Entity type. This name must be unique.'),
      '#required' => TRUE,
      '#size' => 30,
    );
    ...
    $form['type'] = array(
      ...
      '#disabled' => !$type->isNew(),
      '#machine_name' => array(
        'exists' => ['\Drupal\hosting_control_panel\Entity\HostingControlPanelEntityType', 'load'],
        'source' => array('label'),
      ),
      '#description' => $this->t('A unique machine-readable name for this Hosting Control Panel Entity type. It must only contain lowercase letters, numbers, and underscores.'),
    );
    ...
    $form['description'] = array(
      ...
      '#description' => $this->t('Describe this Hosting Control Panel Entity type.'),
    );
    ...
    $form['read_only'] = array(
      ...
      '#description' => $this->t('Wheter or not this Hosting Control Panel Entity type is read only.'),
    );
    ...
    $manager = \Drupal::service('plugin.manager.hosting_control_panel_entity_storage_client');
    ...
    $formats = \Drupal::service('hosting_control_panel_entity.storage_client.response_decoder_factory')->supportedFormats();
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    $actions['submit']['#value'] = t('Save Hosting Control Panel Entity type');
    $actions['delete']['#value'] = t('Delete Hosting Control Panel Entity type');
    return $actions;
  }
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status == SAVED_UPDATED) {
      drupal_set_message(t('The Hosting Control Panel Entity type %name has been updated.', $t_args));
    }
    elseif ($status == SAVED_NEW) {
      drupal_set_message(t('The Hosting Control Panel Entity type %name has been added.', $t_args));
      $context = array_merge($t_args, array('link' => $type->link($this->t('View'), 'collection')));
      $this->logger('hosting_control_panel_entities')->notice('Added Hosting Control Panel Entity type %name.', $context);
    }
